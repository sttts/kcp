...
func (options *Options) AddFlags(fs *pflag.FlagSet) {
	...
	fs.StringVar(&options.PclusterID, "workload-cluster-name", options.PclusterID,
		fmt.Sprintf("ID of the -to cluster. Resources with this ID set in the '%s' label will be synced.", workloadv1alpha1.InternalWorkloadClusterStateLabelPrefix+"<ClusterID>"))

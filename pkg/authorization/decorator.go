	...
	d.target = authorizer.AuthorizerFunc(func(ctx context.Context, attr authorizer.Attributes) (authorizer.Decision, string, error) {
		...
		if domain := ctx.Value(auditDomainKey); domain != nil && domain != "" {
			kaudit.AddAuditAnnotations(
				ctx,
				fmt.Sprintf("%s.%s/%s", d.key, domain, auditDecision), decisionString(dec),
				fmt.Sprintf("%s.%s/%s", d.key, domain, auditReason), auditReasonMsg,
			)
		}
...
// WithAuditLogging sets stores the given domain in the context to be used when logging

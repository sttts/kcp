...
func (s *Authorization) ApplyTo(config *genericapiserver.Config, kubeInformers, globalKubeInformers kcpkubernetesinformers.SharedInformerFactory, kcpInformers, globalKcpInformers kcpinformers.SharedInformerFactory) error {
	...
	bootstrapAuth = authz.NewDecorator("bootstrap", bootstrapAuth).AddAuditLogging().AddAnonymization().AddReasonAnnotation()

	// resolves RBAC resources in the workspace
	localAuth, localResolver := authz.NewLocalAuthorizer(kubeInformers)
	localAuth = authz.NewDecorator("local", localAuth).AddAuditLogging().AddAnonymization().AddReasonAnnotation()

	globalAuth, _ := authz.NewGlobalAuthorizer(kubeInformers, globalKubeInformers)
	globalAuth = authz.NewDecorator("global", globalAuth).AddAuditLogging().AddAnonymization().AddReasonAnnotation()
	...
	maxPermissionPolicyAuth = authz.NewDecorator("maxpermissionpolicy", maxPermissionPolicyAuth).AddAuditLogging().AddAnonymization().AddReasonAnnotation()

	// protect status updates to apiexport and apibinding
	systemCRDAuth := authz.NewSystemCRDAuthorizer(maxPermissionPolicyAuth)
	systemCRDAuth = authz.NewDecorator("systemcrd", systemCRDAuth).AddAuditLogging().AddAnonymization().AddReasonAnnotation()
	...
	contentAuth = authz.NewDecorator("content", contentAuth).AddAuditLogging().AddAnonymization().AddReasonAnnotation()
	...
	requiredGroupsAuth = authz.NewDecorator("requiredgroups", requiredGroupsAuth).AddAuditLogging().AddAnonymization()

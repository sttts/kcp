...
func (r *deletionReconciler) reconcile(ctx context.Context, workspace *tenancyv1beta1.Workspace) (reconcileStatus, error) {
	...
	if _, err := r.getThisWorkspace(ctx, logicalcluster.NewPath(workspace.Status.Cluster.String())); err != nil && !apierrors.IsNotFound(err) {
	...
	if err := r.deleteThisWorkspace(ctx, logicalcluster.NewPath(workspace.Status.Cluster.String())); err != nil {
		return reconcileStatusStopAndRequeue, err
	}

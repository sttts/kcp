...
test-e2e-shared: require-kind build-all build-kind-images
	...
	kind get kubeconfig > "$(WORK_DIR)/kind.kubeconfig"
	rm -f "$(WORK_DIR)/admin.kubeconfig"
	...
	NO_GORUN=1 $(GO_TEST) -race -count $(COUNT) -p $(E2E_PARALLELISM) -parallel $(E2E_PARALLELISM) $(WHAT) $(TEST_ARGS) \
		-args --use-default-kcp-server --syncer-image="$(SYNCER_IMAGE)" --kcp-test-image="$(TEST_IMAGE)" --pcluster-kubeconfig="$(abspath $(WORK_DIR)/kind.kubeconfig)"
...
test-e2e-sharded: require-kind build-all build-kind-images
	...
	while [ ! -f .kcp/admin.kubeconfig ]; do sleep 1; done && \
	NO_GORUN=1 $(GO_TEST) -race -count $(COUNT) -p $(E2E_PARALLELISM) -parallel $(E2E_PARALLELISM) $(WHAT) $(TEST_ARGS) \
		-args --use-default-kcp-server --root-shard-kubeconfig=$(PWD)/.kcp-0/admin.kubeconfig --syncer-image="$(SYNCER_IMAGE)" --kcp-test-image="$(TEST_IMAGE)" --pcluster-kubeconfig="$(abspath $(WORK_DIR)/kind.kubeconfig)"
